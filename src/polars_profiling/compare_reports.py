...
from polars_profiling.config import Correlation, Settings
from polars_profiling.model import BaseDescription
from polars_profiling.model.alerts import Alert
from polars_profiling.profile_report import ProfileReport
...
def _update_titles(reports: List[ProfileReport]) -> None:
    ...
    for idx, report in enumerate(reports):
        if report.config.title == "Polars Profiling Report":
...
def compare(
    ...
    if isinstance(reports[0], ProfileReport):
        ...
        configs_str = [
            json.loads(r.package["polars_profiling_config"]) for r in reports  # type: ignore
        ]
        all_configs = []
