    ...
    def loads(self, data: bytes) -> Union["ProfileReport", "SerializeReport"]:
        ...
        if (df_hash == self.df_hash) or (self.df is None):
            ...
            if (
                ...
                and loaded_description_set.package["polars_profiling_version"]
                != __version__
            ):
                warnings.warn(
                    f"The package version specified in the loaded data is not equal to the version installed. "
                    f"Currently running on data-profiling {__version__} , while loaded data is generated by polars_profiling, {loaded_description_set.package['polars_profiling_version']}."
                )
